function initImageToWebP() {
    ...
    function createImageCard(name, src, size, originalSize, blob) {
        ...
        const colorClass = size < originalSize ? 'size-smaller' : 'size-larger';
        card.innerHTML = `
            <img src="${src}" alt="${name}">
            <div class="image-info">
                <p>${name}</p>
                <p>Original: ${formatSize(originalSize)}</p>
                <p>WebP: ${formatSize(size)} (<span class="${colorClass}">${sizeDiff}%</span>)</p>
            </div>
            <button class="card-download-btn">Download</button>
        `;
        card.querySelector('.card-download-btn').addEventListener('click', () => downloadSingleImage(name, src));
        return card;
    }
